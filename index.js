...
app.get('/health', (req, res) => {
    res.json({ status: botReady ? 'ready' : 'starting', uptime: process.uptime(), timestamp: new Date().toISOString() });
});

app.get('/healthz', (req, res) => {
    const statusCode = botReady ? 200 : 503;
    res.status(statusCode).json({ status: botReady ? 'ok' : 'starting', uptime: process.uptime(), timestamp: new Date().toISOString() });
});
